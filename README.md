...
- **Dashboard stats widget** (`Widgets\AttendanceOverviewWidget`) — today's present/late/absent breakdown (or today's punch count, if you haven't enabled Attendance Summaries), active employee count, and a combined pending-approvals count across Leave/Overtime/Corrections. Feature-gated the same way as every resource, and query-cached for 30s so a dashboard several admins keep open doesn't turn into a `count()` on every page load.
